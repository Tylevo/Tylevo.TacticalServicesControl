...
| `HelicopterItemTransferIncludedFeePurchasePatch` | Replaces the retired stash-fee purchase interception in 1.3.13 | `ItemManipulator.PurchaseTraderService(GlobalConfiguration.ServiceData, string, EFT.Quests.QuestController, InventoryController, bool)` | Copies service metadata with empty costs for the exact UH-60 session; native simulation/deferred execution retain the copy; in-game acceptance pending |
| `HelicopterItemTransferIncludedFeeQuotePatch` | Replaces the retired stash-fee button override in 1.3.13 | Concrete `TransferItemsController.GetGridItemsPrice(Stash, ETraderServiceType, float, float) -> int` | Scoped zero quote feeds native display/affordability while preserving the empty-grid guard; in-game acceptance pending |
