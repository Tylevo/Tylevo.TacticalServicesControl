...
| P3-03B | Configurable HUD presentation | In F12, switch `UAV Radar Display > Display mode` between `Phone` and `HUD` before and during an active recon link. In HUD mode, test all four `HUD position` values, inventory open/close, and multiple screen aspect ratios. | Phone remains the default and preserves the held-J behavior. HUD mode creates one requester-local, non-interactive square containing only the live scanner, sweep, orientation labels, player marker, and contact blips; no phone header, status bands, telemetry, footer, or surrounding phone panel is visible. It follows position changes live, hides over inventory, returns afterward, and is destroyed at expiry/death/raid teardown. Switching Phone -> HUD safely restores any raised phone before the HUD appears. | OPEN |
