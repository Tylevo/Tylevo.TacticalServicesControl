...
| P4-C06 | Schema/default migration | Test the exact published schema-less v1.0.8 config with standard dispatch `0`, a schema-less custom nonzero standard dispatch, a schema-less missing/null Extraction section, a clean install with both canonical and legacy config absent, and a schema-2 explicit standard dispatch `0`. | Every legacy input saves as schema 3. Schema-less standard values retain the historical effective `8`-second delay and missing/null sections receive safe defaults. Pre-currency inputs default to `RUB` without converting numeric prices. A clean install creates canonical schema 3 with standard `8` and priority `3`. A schema-2 explicit `0` survives migration/restart and dispatches immediately. | OPEN |
