...
The active v1.3.3 candidate adds native phone purchase screens and hold-Alt
mouse selection, retaining the eased zoom and ballistic corrections.
Build, regression, and package results are recorded in the candidate's
external evidence sidecars; in-game phone acceptance remains pending. See
`docs/release-notes-v1.3.3.md` for controls and acceptance scope.
...
The v1.3.3 package contract follows the verified public v1.0.8 artifact:
...
powershell -NoProfile -ExecutionPolicy Bypass -File .\tools\verify-local.ps1 `
  -SptDir "C:\Path\To\SPT" `
  -SptSharedAssembliesDir "C:\Path\To\SPT Assemblies" `
  -EvidencePath "C:\External\TSC\v1.3.3-build-evidence.json"
```
...
powershell -NoProfile -ExecutionPolicy Bypass -File .\tools\New-ReleasePackage.ps1 `
  ...
  -OutputDirectory "C:\External\TSC\v1.3.3-candidate" `
  -BuildEvidencePath "C:\External\TSC\v1.3.3-build-evidence.json"
```
...
`AssemblyVersion`/`FileVersion` `1.3.3.0`, and
`AssemblyInformationalVersion` `1.3.3+<current-clean-HEAD>`. This rejects old,
...
`Tylevo.TacticalServicesControl-v1.3.3-SPT4.1.4-TESTER.zip`. The `TESTER`
