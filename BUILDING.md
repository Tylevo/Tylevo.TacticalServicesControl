...
The active v1.3.5 candidate refreshes the store and phone icon assets with
the supplied tactical artwork. Runtime code and layouts are unchanged.
...
`docs/release-notes-v1.3.5.md` for scope and acceptance.
...
The v1.3.5 package contract follows the verified public v1.0.8 artifact:
...
powershell -NoProfile -ExecutionPolicy Bypass -File .\tools\verify-local.ps1 `
  -SptDir "C:\Path\To\SPT" `
  -SptSharedAssembliesDir "C:\Path\To\SPT Assemblies" `
  -EvidencePath "C:\External\TSC\v1.3.5-build-evidence.json"
```
...
powershell -NoProfile -ExecutionPolicy Bypass -File .\tools\New-ReleasePackage.ps1 `
  ...
  -OutputDirectory "C:\External\TSC\v1.3.5-candidate" `
  -BuildEvidencePath "C:\External\TSC\v1.3.5-build-evidence.json"
```
...
`AssemblyVersion`/`FileVersion` `1.3.5.0`, and
`AssemblyInformationalVersion` `1.3.5+<current-clean-HEAD>`. This rejects old,
...
`Tylevo.TacticalServicesControl-v1.3.5-SPT4.1.4-TESTER.zip`. The `TESTER`
