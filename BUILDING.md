...
The active v1.3.4 candidate redesigns the pre-raid Uplink storefront, retaining
the native phone UI, Alt selection, eased zoom, and ballistic corrections.
Build, regression, and package results are recorded in the candidate's
external evidence sidecars; live storefront acceptance remains separate. See
`docs/release-notes-v1.3.4.md` for controls and acceptance scope.
...
The v1.3.4 package contract follows the verified public v1.0.8 artifact:
...
powershell -NoProfile -ExecutionPolicy Bypass -File .\tools\verify-local.ps1 `
  -SptDir "C:\Path\To\SPT" `
  -SptSharedAssembliesDir "C:\Path\To\SPT Assemblies" `
  -EvidencePath "C:\External\TSC\v1.3.4-build-evidence.json"
```
...
powershell -NoProfile -ExecutionPolicy Bypass -File .\tools\New-ReleasePackage.ps1 `
  ...
  -OutputDirectory "C:\External\TSC\v1.3.4-candidate" `
  -BuildEvidencePath "C:\External\TSC\v1.3.4-build-evidence.json"
```
...
`AssemblyVersion`/`FileVersion` `1.3.4.0`, and
`AssemblyInformationalVersion` `1.3.4+<current-clean-HEAD>`. This rejects old,
...
`Tylevo.TacticalServicesControl-v1.3.4-SPT4.1.4-TESTER.zip`. The `TESTER`
